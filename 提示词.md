        ...
        <div class="flex items-stretch mb-6">
             ...
             <div class="flex-grow pr-8 md:pr-0">
                <h2 class="text-3xl font-serif font-bold text-gray-800 flex items-center js-day-title">
                    ...
                    <span class="text-gray-400 font-light select-none">|</span>
                    <span contenteditable="true" class="...">标题...</span>
                </h2>
                <!-- 天气部分保持不变 -->
            </div>
        </div>
...
*   **添加新卡片按钮 (Add Button - Revised Style)：**
    *   **样式要求：** 必须更改为 **虚线边框** 风格，不再使用之前的毛玻璃样式。
    *   **强制 HTML 结构：**
        ```html
        <button onclick="window.addTimelineCard(this.previousElementSibling)" data-html2canvas-ignore="" class="group w-full py-3 mt-4 border-2 border-dashed border-gray-200 rounded-xl text-gray-400 hover:text-wuxi-main hover:border-wuxi-main transition-colors flex items-center justify-center text-sm">
...
**每个时间节点卡片都应该随机一个卡片颜色**
**类型A：普通时间节点卡片 (Standard Timeline Card)**
请严格复刻以下 HTML 结构和 CSS 类名逻辑：

*   **配色系统 (Color Themes):**
    *   **逻辑要求：** JS 必须定义 `CARD_THEMES` 数组。
    *   **对象结构更新：** 为了匹配 HTML 中的文字颜色区分，主题对象需包含：
        *   `bg`: 背景色 (e.g., `bg-teal-50/40`)
        *   `border`: 外边框色 (e.g., `border-teal-200`)
        *   `textPrimary`: 时间/强文本色 (e.g., `text-teal-800`)
        *   `textSecondary`: 地点/弱文本色 (e.g., `text-teal-600`)
    *   **切换逻辑：** `window.cycleCardColor(btn)` 需同时更新卡片背景、边框以及 Header 中的两组文字颜色。

*   **HTML 结构规范：**

    1.  **外层容器 (Card Container):**
        *   类名必须包含：`js-timeline-card relative group/card rounded-2xl border p-6`。
        *   **注意：** 圆角统一为 `rounded-2xl`，背景色需带透明度 (e.g., `/40`)。

    2.  **顶部悬浮工具栏 (Top Floating Toolbar):**
        *   **位置：** 绝对定位 `top-4 right-4` (原提示词为 top-2，需修正)。
        *   **样式：** `bg-white/80 backdrop-blur rounded-lg p-1`，默认 `opacity-0`，Hover 时 `group-hover/card:opacity-100`。
        *   **按钮风格：** 不再使用 SVG 代码，改用 **Emoji/字符图标** (`✥`, `🎨`, `✕`)。
        *   **拖拽手柄：** 必须是一个 `<button>` 标签，且包含类名 `card-drag-handle` 和 `cursor-move`。
        *   **代码参考：**
            ```html
            <div class="absolute top-4 right-4 flex gap-2 opacity-0 group-hover/card:opacity-100 transition-opacity no-print bg-white/80 rounded-lg p-1 backdrop-blur">
                <button class="w-6 h-6 text-xs rounded hover:bg-gray-100 cursor-move card-drag-handle">✥</button>
                <button onclick="window.cycleCardColor(this)" class="w-6 h-6 text-xs rounded hover:bg-gray-100">🎨</button>
                <button onclick="window.confirmDelete(this.closest('.js-timeline-card'))" class="w-6 h-6 text-xs rounded hover:bg-red-100 text-red-500">✕</button>
            </div>
            ```

    3.  **头部信息 (Header):**
        *   **布局：** Flex 布局 `flex items-center gap-2`。
        *   **下划线处理：** 使用 `border-b` (或通过颜色类定义边框) + `pb-2 mb-3` 来形成视觉分割。
        *   **时间样式：** `font-mono font-bold text-lg` + 主题深色 (`text-teal-800`)。
        *   **地点样式：** `text-xs font-bold uppercase tracking-wider` + 主题次深色 (`text-teal-600`)。

    4.  **内容主体 (Body):**
        *   **标题：** `h3` 标签，样式 `text-xl font-serif font-bold text-gray-800 mb-2`。
        *   **内容列表容器：** `<div class="content-list space-y-2">`。
        *   **默认文本块：**
            *   必须包含删除按钮 (`absolute -right-2 -top-2`)，默认隐藏 Hover 显示。
            *   文本框样式：`p-3 bg-white/50 rounded-lg text-sm text-gray-600`，边框透明 Hover 显灰。

    5.  **底部快速插入栏 (Quick Insert Toolbar):**
        *   **说明：** 这是一个新增的重要结构，原提示词未包含。
        *   **样式：** `mt-4 pt-2 border-t border-dashed border-gray-300 flex gap-2 overflow-x-auto`。
        *   **功能：** 包含 `📝文本`, `🖼️相册`, `📷机位`, `⚠️提醒`, `🎫门票`, `🚗交通` (交通按钮需特殊高亮样式：`bg-blue-50 text-blue-600`)。
        *   **交通卡片插入按钮：** 需特殊高亮，样式为 `bg-blue-50 text-blue-600 border-blue-200`，点击触发 `window.insertTimelineCard(..., 'transit')`。
*   **HTML 结构 (必须包含拖拽手柄):**
    ```html
	<!-- Type A 卡片结构模板 -->
	<div class="js-timeline-card relative group/card rounded-2xl border border-teal-200 bg-teal-50/40 p-6">
		
		<!-- 1. 顶部悬浮工具栏 (默认隐藏，Hover显示) -->
		<div class="absolute top-4 right-4 flex gap-2 opacity-0 group-hover/card:opacity-100 transition-opacity no-print bg-white/80 rounded-lg p-1 backdrop-blur">
			<button class="w-6 h-6 text-xs rounded hover:bg-gray-100 cursor-move card-drag-handle">✥</button>
			<button onclick="window.cycleCardColor(this)" class="w-6 h-6 text-xs rounded hover:bg-gray-100">🎨</button>
			<button onclick="window.confirmDelete(this.closest('.js-timeline-card'))" class="w-6 h-6 text-xs rounded hover:bg-red-100 text-red-500">✕</button>
		...
		<!-- 2. Header: 时间与地点 -->
		<div class="flex items-center gap-2 border-teal-200 pb-2 mb-3">
			<span class="font-mono font-bold text-teal-800 text-lg" contenteditable="true">
				<!-- Insert Time Here (e.g. 09:00) -->
			</span>
			<span class="text-xs font-bold uppercase tracking-wider text-teal-600" contenteditable="true">
				<!-- Insert Location Here -->
			</span>
		</div>

		<!-- 3. Title: 标题 -->
		<h3 class="text-xl font-serif font-bold text-gray-800 mb-2" contenteditable="true">
			<!-- Insert Title Here -->
		</h3>
		...
		<div class="content-list space-y-2">
			<!-- 默认文本块 (可删除) -->
			<div class="js-content-block relative group/item">
				<button onclick="this.parentElement.remove()" class="absolute -right-2 -top-2 text-red-400 opacity-0 group-hover/item:opacity-100 no-print">×</button>
				<div class="p-3 bg-white/50 rounded-lg text-sm text-gray-600 leading-relaxed border border-transparent hover:border-gray-200" contenteditable="true">
					<!-- Insert Initial Text Here -->
				</div>
			</div>
		</div>

		<!-- 5. Quick Insert: 底部工具栏 -->
		<div class="mt-4 pt-2 border-t border-dashed border-gray-300 flex gap-2 overflow-x-auto no-print opacity-50 hover:opacity-100 transition-opacity">
			<button onclick="window.addContent(this, 'text')" class="text-xs px-2 py-1 bg-white rounded border hover:bg-gray-50">📝文本</button>
			<button onclick="window.addContent(this, 'photo')" class="text-xs px-2 py-1 bg-white rounded border hover:bg-gray-50">🖼️相册</button>
			<button onclick="window.addContent(this, 'camera')" class="text-xs px-2 py-1 bg-white rounded border hover:bg-gray-50">📷机位</button>
			<button onclick="window.addContent(this, 'warning')" class="text-xs px-2 py-1 bg-white rounded border hover:bg-gray-50">⚠️提醒</button>
			<button onclick="window.addContent(this, 'ticket')" class="text-xs px-2 py-1 bg-white rounded border hover:bg-gray-50">🎫门票</button>
			<button onclick="window.insertTimelineCard(this.closest('.js-timeline-card'), 'transit')" class="text-xs px-2 py-1 bg-blue-50 text-blue-600 rounded border border-blue-200 hover:bg-blue-100 font-bold">🚗交通</button>
		</div>
	</div>
    ```
...
6.  **Toast 反馈：** 在 `addNewDay`, `saveData` (自动保存), `deleteItem` 操作完成后，调用 `showToast` 给予用户视觉反馈。
7.  **导出处理：**
